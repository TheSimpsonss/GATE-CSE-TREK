...
// Use the Subject model from models/Subject.js if available, otherwise define it
// This prevents "Cannot overwrite model" errors when seed.js is required
let Subject;
try {
  ...
  // If model doesn't exist, define it (for standalone seed script)
...
// Export INITIAL_SYLLABUS for use in routes
module.exports.INITIAL_SYLLABUS = INITIAL_SYLLABUS;
...
async function seedDatabase() {
  try {
    ...
    // Get Subject model (ensure it's available)
    if (!Subject) {
      Subject = mongoose.models.Subject || require('./models/Subject');
    }

    // Clear existing data (only if running standalone seed)
    // Note: In production, this should be user-specific
    await Subject.deleteMany({});
    console.log('Cleared existing subjects');

    // Insert initial syllabus
    await Subject.insertMany(INITIAL_SYLLABUS);
    console.log(`Seeded ${INITIAL_SYLLABUS.length} subjects successfully!`);
...
// Only run seedDatabase if this file is executed directly (not when required)
